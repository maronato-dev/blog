...
import { useSearchAsYouType } from "../../../hooks/search"
...
export default defineComponent({
  ...
  setup() {
    ...
    const { results, noResults } = useSearchAsYouType(query)
    const i18n = useI18n()
    const localizedResults = computed(() =>
      results.value
        .filter(result => result.doc.language === i18n.locale.value)
        .slice(0, 5)
    )
    ...
    const selectFocus = () => {
      if (localizedResults.value.length > 0) {
        ...
        const slug = focused.doc.slug
        if (focused.table === "tags") {
